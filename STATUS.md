...
- **Session close ritual (verbal, not in PROTOCOL.md yet):** Owner says "close" → agent summarizes what's done/pending → owner approves → agent applies to docs + commits + pushes + updates STATUS + marks next steps. This file is the artifact; PROTOCOL.md should probably get this ritual baked in next session.
